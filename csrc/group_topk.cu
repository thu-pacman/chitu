...
__launch_bounds__(TPB) __global__ void group_topk_kernel_gather_weights(
    ...
    const torch::BFloat16* __restrict__ original_scores,
    float route_scale,
    float* weights,
    int64_t* indices,
    ...
    for(size_t group_idx = 0; group_idx < NUM_GROUPS; group_idx ++) {
        ...
        for(int64_t k_idx = 0; k_idx < 2; k_idx++) {
            ...
            for(int i = 0; i < group_dim; ++i) {
                size_t idx = cur_token_idx * NUM_GROUPS * group_dim + group_idx * group_dim + i;
                inp_kvp.key = i;
    ...
    for(size_t topk_group_idx = 0; topk_group_idx < TOPK_GROUPS; ++topk_group_idx) {
        thread_kvp.key = 0;
        thread_kvp.value = -1.f;
        cub_kvp inp_kvp;
        for(size_t group_idx = 0; group_idx < NUM_GROUPS; group_idx ++) {
    ...
    for(size_t topK_idx = 0; topK_idx < TOPK; ++topK_idx) {
        ...
        for(size_t topk_group_idx = 0; topk_group_idx < TOPK_GROUPS; ++topk_group_idx) {
            for(size_t i = 0; i < group_dim; ++i) {
                size_t group_idx = topk_group_indices[topk_group_idx] * group_dim +i;
...
void groupTopKLauncher(torch::Tensor &input_after_act, torch::Tensor &original_scores, float route_scale, torch::Tensor &weights, torch::Tensor &indices, const size_t num_groups,cudaStream_t stream) {
    const int TPB = 1;
    ...
    group_topk_kernel_gather_weights<TPB, 8, 4, 8><<<num_blocks, TPB, 0, stream>>>(
        input_after_act.data_ptr<float>(),
        original_scores.data_ptr<torch::BFloat16>(),
        route_scale,
...
void groupTopKIndices(torch::Tensor &input_after_act, torch::Tensor &original_scores, float route_scale, torch::Tensor &weights, torch::Tensor &indices, const int num_groups) {
    ...
    groupTopKLauncher(input_after_act, original_scores, route_scale, weights, indices, num_groups, stream);
}
