...
void Linear::warm_up(CPUInfer *CPUInfer) {
...
void Linear::forward_many(int qlen, const void* input, void* output, CPUInfer *CPUInfer) {
...
void Linear::forward(int qlen, const void* input, void* output, CPUInfer *CPUInfer) {
    if (qlen <= 0) {
        return;
    }
    int forward_len = std::min(qlen, config_.group_max_len);
    forward_many(forward_len, input, output, CPUInfer);
    forward(qlen - forward_len, (uint8_t*)input + forward_len * config_.input_size * ggml_type_size(config_.hidden_type) / ggml_blck_size(config_.hidden_type), (uint8_t*)output + forward_len * config_.output_size * ggml_type_size(config_.hidden_type) / ggml_blck_size(config_.hidden_type), CPUInfer);
}