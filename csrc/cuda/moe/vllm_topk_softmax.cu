 ...
 * - The kernel to topk_softmax for MoE is originally from vLLM
 *   (https://github.com/vllm-project/vllm/blob/main/csrc/moe/topk_softmax_kernels.cu),
 *   licensed under Apache 2.0.
 *
 * The sources have further copyright notices:
 *
 * Adapted from
...
__launch_bounds__(TPB) __global__
    ...
    for (int ii = threadIdx.x; ii < num_cols; ii += TPB) {
        const int idx = thread_row_offset + ii;
        const float val = exp((static_cast<float>(input[idx]) - float_max)) *
                          normalizing_factor;
        output[idx] = val;
    }
}
...
__launch_bounds__(TPB) __global__
    ...
    for (int k_idx = 0; k_idx < k; ++k_idx) {
        ...
        if (threadIdx.x == 0) {
            // Ignore experts the node isn't responsible for with expert
            // parallelism
...
#pragma unroll
        ...
        if (thread_group_idx == 0) {
            ...
            // results to global memory. (This will be a single) thread per row
            // of the input/output matrices.
        ...
        if (k_idx + 1 < k) {
            ...
            if (thread_group_idx == thread_to_clear_in_group) {
                ...
                // be between 0 and 1.
                row_chunk[ldg_group_for_expert * ELTS_PER_LDG +
...
// compile time.
template <int EXPERTS, int BYTES_PER_LDG> struct TopkConstants {
    static constexpr int ELTS_PER_LDG = BYTES_PER_LDG / sizeof(float);
...
void topkGatingSoftmaxLauncherHelper(const float *input, const bool *finished,
    ...
    topkGatingSoftmax<VPT, EXPERTS, WARPS_PER_TB, BYTES_PER_LDG>
        <<<num_blocks, block_dim, 0, stream>>>(input, finished, output,
                                               num_rows, indices, source_row, k,
...
void topkGatingSoftmaxKernelLauncher(
    const float *gating_output, float *topk_weights, IndType *topk_indicies,
                  ...
                  torch::Tensor &gating_output) // [num_tokens, num_experts]
{
    ...
    assert(topk_indices.scalar_type() == at::ScalarType::Int);
    topkGatingSoftmaxKernelLauncher(
        ...
        stream);
    // }
    // this code will cause "error: no member named 'UInt32' in
    // 'c10::ScalarType'; did you mean 'QInt32'?" on muxi
    // else {
    //     assert(topk_indices.scalar_type() == at::ScalarType::UInt32);
    //     topkGatingSoftmaxKernelLauncher(gating_output.data_ptr<float>(),
    //                                     topk_weights.data_ptr<float>(),
    //                                     !!! here will cause undefined symbol:
    //                                     _ZNK2at10TensorBase8data_ptrIjEEPT_v
    //                                     in muxi runtime !!!
