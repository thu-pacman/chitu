...
def remove_old_kernels():
    # Remove all old kernel_*.cu files AND the new target files
    pattern = os.path.dirname(__file__) + "/kernel_*.cu"
    for filename in glob.glob(pattern):
        subprocess.call(["rm", "-f", filename])
    # Also explicitly remove the two target files in case they exist
    for target in ["fp16_kernel.cu", "bf16_kernel.cu"]:
...
def generate_new_kernels():
    # Prepare accumulators for each dtype
