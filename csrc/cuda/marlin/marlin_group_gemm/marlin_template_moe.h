...
#ifndef MARLIN_MOE_NAMESPACE_NAME
#define MARLIN_MOE_NAMESPACE_NAME marlin_moe_wna16
#endif
...
namespace MARLIN_MOE_NAMESPACE_NAME {
...
#pragma unroll
        for (int i = 0; i < moe_block_size / 4; i++) {
            ...
            int *sorted_token_ids =
                reinterpret_cast<int *>(&sorted_token_ids_int4);
#pragma unroll
            for (int j = 0; j < 4; j++) {
...
#pragma unroll
    ...
    auto update_next_moe_block_data = [&]() {
        ...
        if (num_invalid_blocks > 0) {
            ...
            block_id++;
            for (int i = block_id; i < num_tokens_past_padded / moe_block_size;
                 i++) {
...
#pragma unroll
                for (int k = 0; k < b_sh_wr_iters; k++) {
                    int idx = (pipe >= stages &&
                               stage_group_id == max_num_stage_groups - 1)
                                  ? (pipe - stages)
