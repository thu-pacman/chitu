...
__global__ void MarlinDefault(MARLIN_KERNEL_PARAMS_MOE){};

using MarlinFuncPtr = void (*)(MARLIN_KERNEL_PARAMS_MOE);

#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ < 800

template <int moe_block_size>
__global__ void
permute_cols_kernel(int4 const *__restrict__ a_int4_ptr,
...
int get_kernel_cache_size(thread_config_t const &th_config, bool m_block_size_8,
    ...
    int sh_g_idx_size =
        has_act_order && !is_k_full ? pipe_stages * tb_k / 4 : 0;
    int sh_zp_size = 0;
    if (has_zp) {
        if (is_zp_float)
        ...
        constexpr auto S_TYPE =                                                \
            W_TYPE == vllm::kFE2M1f                                            \
                ...
                : (std::is_same<scalar_t, half>::value ? vllm::kFloat16        \
                                                       : vllm::kBFloat16);     \
        kernel =                                                               \
            Marlin<scalar_t, W_TYPE.id(), S_TYPE.id(), NUM_THREADS,            \
...
MarlinFuncPtr
get_marlin_kernel(const vllm::ScalarType q_type, int thread_m_blocks,
                  int thread_n_blocks, int thread_k_blocks, bool m_block_size_8,
                  bool has_act_order, bool has_zp, int group_blocks,
                  int num_threads, bool is_zp_float) {
...
void marlin_mm(const void *A, const void *B, void *C, void *C_tmp, void *b_bias,
    ...
    if (has_zp) {
        ...
        TORCH_CHECK(q_type == vllm::kU4B8 || q_type == vllm::kU8B128 ||
                    ...
                    "q_type must be uint4b8, uint8b128, float8_e4m3fn or "
                    "float4_e2m1f when "
                    "has_zp = False. Got = ",
                    q_type.str());
    }
    ...
    kernel<<<sms, default_threads, 0, stream>>>(
        ...
        // If we have a full K, then we can run the non-act-order version of
        // Marlin (since the weight rows are reordered by increasing group ids,
        // and by having a full K, we have full original groups)
        if (is_k_full)
            has_act_order = false;
    }
...
torch::Tensor moe_wna16_marlin_gemm(
    ...
    // Verify B
    TORCH_CHECK(size_k % MARLIN_MOE_NAMESPACE_NAME::tile_size == 0,
                "size_k = ", size_k, " is not divisible by tile_size = ",
                MARLIN_MOE_NAMESPACE_NAME::tile_size);
    TORCH_CHECK((size_k / MARLIN_MOE_NAMESPACE_NAME::tile_size) ==
                    b_q_weight.size(1),
                "Shape mismatch: b_q_weight.size(1) = ", b_q_weight.size(1),
                ", size_k = ", size_k,
                ", tile_size = ", MARLIN_MOE_NAMESPACE_NAME::tile_size);
    TORCH_CHECK(b_q_weight.size(2) % MARLIN_MOE_NAMESPACE_NAME::tile_size == 0,
                "b_q_weight.size(2) = ", b_q_weight.size(2),
                " is not divisible by tile_size = ",
                MARLIN_MOE_NAMESPACE_NAME::tile_size);
    int actual_size_n =
        (b_q_weight.size(2) / MARLIN_MOE_NAMESPACE_NAME::tile_size) *
        pack_factor;
    ...
    if (use_fp32_reduce && !use_atomic_add) {
        ...
        long max_c_tmp_size = min((long)size_n * sorted_token_ids.size(0),
                                  (long)sms * 4 * moe_block_size *
                                      MARLIN_MOE_NAMESPACE_NAME::max_thread_n);
        if (moe_block_size == 8)
            max_c_tmp_size *= 2;
    ...
    if (global_scale_or_none.has_value()) {
        ...
        TORCH_CHECK(
            !(b_q_type == vllm::kFE2M1f && group_size == 16),
    ...
    TORCH_CHECK(size_n % MARLIN_MOE_NAMESPACE_NAME::min_thread_n == 0,
                "size_n = ", size_n, ", is not divisible by min_thread_n = ",
                MARLIN_MOE_NAMESPACE_NAME::min_thread_n);

    int max_n_tiles = size_n / MARLIN_MOE_NAMESPACE_NAME::min_thread_n;
    int min_workspace_size =
        min(max_n_tiles * (int)(sorted_token_ids.size(0) / moe_block_size),
            sms * 4);
    ...
    if (a.scalar_type() == at::ScalarType::Half) {
        ...
        MARLIN_MOE_NAMESPACE_NAME::marlin_mm<half>(
            a.data_ptr<at::Half>(), b_q_weight.data_ptr(),
            c.data_ptr<at::Half>(), c_tmp.data_ptr<float>(),
            b_bias.data_ptr<at::Half>(), scales_ptr,
            global_scale.data_ptr<at::Half>(), b_zeros.data_ptr(),
            g_idx.data_ptr(), perm.data_ptr(), a_tmp.data_ptr<at::Half>(),
            sorted_token_ids.data_ptr(), expert_ids.data_ptr(),
            num_tokens_past_padded.data_ptr(), topk_weights.data_ptr(),
            moe_block_size, top_k, mul_topk_weights, is_ep, size_m, size_n,
            size_k, workspace.data_ptr(), b_q_type, has_bias, has_act_order,
        ...
        MARLIN_MOE_NAMESPACE_NAME::marlin_mm<nv_bfloat16>(
