// Instantiated by the three small CUDA translation units. Algorithm ported
// from hygon_indexer_topk.cu; do not include this implementation twice in one
// TU. SPDX-FileCopyrightText: 2025 Qingcheng.AI
//
...
namespace chitu::NV_TOPK_VARIANT {

namespace {

constexpr int kTopK = 2048;
constexpr int kThreads = NV_TOPK_THREADS;
constexpr int kWriteThreads = 1024;
constexpr int kRadix = 256;
constexpr int kFloatCoarseBits = 12;
constexpr int kFloatLocalCandidateCapacity = NV_LOCAL_CAPACITY;
constexpr int kFloatMergeCandidateCapacity = 3072;
// FP16/BF16 path: exact two-pass radix selection over the 16-bit sortable key.
struct TopKParams {
...
__device__ __forceinline__ void select_histogram_threshold(
    ...
    if (tid < kRadix) {
        ...
        for (int offset = 1; offset < warpSize; offset <<= 1) {
            const auto previous =
                __shfl_up_sync(0xffffffffu, inclusive, offset, warpSize);
            if (lane >= offset) {
                inclusive += previous;
            }
        }
...
#pragma unroll
        for (int offset = GroupSize / 2; offset > 0; offset >>= 1) {
            total += __shfl_down_sync(0xffffffffu, total, offset, GroupSize);
        }
        if (lane_in_group == 0) {
...
template <int MaxItems>
__global__
__launch_bounds__(kThreads) void nvidia_indexer_topk_short(TopKParams params) {
...
__global__
__launch_bounds__(kThreads) void nvidia_indexer_topk(TopKParams params) {
...
// Short direct calls use one warp to scan 256 bins. The original scan
// remains unchanged for long rows and the fixed-graph decode kernels.
template <bool Short>
__device__ __forceinline__ void
select_p1_histogram_threshold(const int *histogram, int target, int *threshold,
                              int *above, int *totals, int *offsets) {
    if constexpr (!Short) {
        select_histogram_threshold(histogram, target, threshold, above, totals,
                                   offsets);
    } else {
...
#pragma unroll
            for (int j = 0; j < 8; ++j) {
                local[j] = histogram[255 - lane * 8 - j];
                total += local[j];
            }
            int incl = total;
#pragma unroll
            for (int shift = 1; shift < 32; shift *= 2) {
                int v = __shfl_up_sync(0xffffffffu, incl, shift);
                if (lane >= shift)
                    incl += v;
            }
            int prefix = incl - total;
            if (prefix < target && incl >= target) {
#pragma unroll
                for (int j = 0; j < 8; ++j) {
                    if (prefix < target && prefix + local[j] >= target) {
                        *threshold = 255 - lane * 8 - j;
                        *above = prefix;
                    }
                    prefix += local[j];
                }
...
template <int CoarseBits, int CandidateCapacity, bool Short = false>
__global__
__launch_bounds__(kThreads) void nvidia_indexer_topk_float_wide_coarse(
...
#pragma unroll
    ...
    __syncthreads();
    select_p1_histogram_threshold<Short>(scratch.group_histogram, kTopK,
                                         &coarse_group, &group_strict_count,
                                         wave_totals, wave_offsets);
    if (tid == 0) {
...
#pragma unroll
        for (int pass = 0; pass < 3; ++pass) {
            ...
            if constexpr (Short) {
                // Every remaining candidate is needed: no further radix
                // refinement or tie selection can change this exact set.
                if (current_count == remaining) {
                    for (int item = tid; item < current_count; item += kThreads)
                        output[exact_strict_count + item] =
                            scratch.candidate_indices[current_buffer][item];
                    return;
                }
            }
            select_p1_histogram_threshold<Short>(
                histogram, remaining, &threshold_byte, &pass_greater_count,
                wave_totals, wave_offsets);
            if (tid == 0) {
...
#pragma unroll
    for (int pass = 0; pass < 4; ++pass) {
        ...
        __syncthreads();
        select_p1_histogram_threshold<Short>(
            histogram, kTopK - exact_strict_count, &threshold_byte,
            &pass_greater_count, wave_totals, wave_offsets);
        if (tid == 0) {
            exact_strict_count += pass_greater_count;
...
// nvidia_indexer_topk_float_wide_coarse<CoarseBits, CandidateCapacity>.  The
// row is explicit only so the unified graph kernel can run the legacy P1 path
// without launching a second node.
template <int CoarseBits, int CandidateCapacity, bool Resume = false,
          bool Short = false>
...
#pragma unroll
        for (int pass = 0; pass < 3; ++pass) {
            ...
            if constexpr (Short) {
                if (current_count == remaining) {
                    for (int item = tid; item < current_count;
                         item += kThreads) {
                        output[scratch.exact_strict_count + item] =
                            scratch.candidates
                                .candidate_indices[current_buffer][item];
                    }
                    return;
                }
            }
    ...
    if (tid == 0) {
        // An overflowing candidate bucket still preserves its exact coarse
        // prefix/count. The dedicated P1 path refines only the remaining bits.
        scratch.prefix = Resume ? uint32_t(scratch.coarse_threshold)
                                      << remaining_low_bits
                                : 0;
        scratch.prefix_mask = Resume ? 0xffffffffu << remaining_low_bits : 0;
        scratch.exact_strict_count = Resume ? scratch.coarse_strict_count : 0;
    }
    __syncthreads();
#pragma unroll
    for (int pass = 0; pass < (Resume ? 3 : 4); ++pass) {
        if (tid < kRadix) {
            scratch.histogram[tid] = 0;
        }
        __syncthreads();
        const auto shift =
            Resume ? (pass == 0 ? remaining_low_bits - 8
                                : (pass == 1 ? remaining_low_bits - 16 : 0))
                   : 24 - pass * 8;
        const auto radix_mask = Resume && pass == 2 ? final_radix_mask : 0xffu;
...
// Used only when every context bucket admitted by the static shape is P1.
// The row's live length selects coarse radix inside this single graph node;
// the two scratch views alias, and no inter-CTA candidates/counters are used.
__global__ __launch_bounds__(
    kThreads,
    2) void nvidia_indexer_topk_float_pure_p1(TopKFloatParams params) {
    union Scratch {
        LegacyFloatWideCoarseScratch<10, kFloatLocalCandidateCapacity>
            short_row;
        LegacyFloatWideCoarseScratch<12, kFloatLocalCandidateCapacity> long_row;
    };
    __shared__ Scratch scratch;
    const auto row = static_cast<int64_t>(blockIdx.x);
    const auto length =
        params.lengths ? params.lengths[row] : params.max_length;
    if (length <= 4096) {
        select_float_topk_legacy_wide_coarse_row<
            10, kFloatLocalCandidateCapacity, true, true>(params, row,
                                                          scratch.short_row);
    } else {
        select_float_topk_legacy_wide_coarse_row<
            12, kFloatLocalCandidateCapacity, true>(params, row,
                                                    scratch.long_row);
    }
}
...
// A compact exact selector shared by FP16 and BF16. Both formats have the
// IEEE sign bit layout needed by sortable_key(), so their raw 16-bit keys can
// be selected without converting the score tensor or changing its ranking.
struct TopK16BlockScratch {
    alignas(128) int histogram[kRadix];
    int threshold_high;
    int threshold_low;
    int remaining_in_high_bin;
    int strict_greater_count;
    int low_strict_greater_count;
    int equal_needed;
    int greater_output_count;
    int equal_output_count;
    int wave_totals[8];
...
template <typename Accessor, typename Writer>
__device__ void select_topk16_block_exact(const Accessor &input,
                                          const Writer &writer, int target,
                                          TopK16BlockScratch &scratch) {
    ...
    for (Slot slot = static_cast<Slot>(tid); slot < slots;
        ...
        if (input.load(slot, key, index)) {
            atomicAdd(&scratch.histogram[key >> 8], 1);
        }
    }
    __syncthreads();
    select_histogram_threshold(scratch.histogram, target,
                               &scratch.threshold_high,
                               &scratch.strict_greater_count,
                               scratch.wave_totals, scratch.wave_offsets);
    if (tid == 0) {
        scratch.remaining_in_high_bin = target - scratch.strict_greater_count;
    }
    __syncthreads();
    ...
    __syncthreads();
    const auto selected_high = scratch.threshold_high;
    for (Slot slot = static_cast<Slot>(tid); slot < slots;
         slot += static_cast<Slot>(kThreads)) {
        uint32_t key;
        int32_t index;
        if (input.load(slot, key, index) && (key >> 8) == selected_high) {
            atomicAdd(&scratch.histogram[key & 0xffu], 1);
        }
    }
    __syncthreads();
    select_histogram_threshold(scratch.histogram, scratch.remaining_in_high_bin,
                               &scratch.threshold_low,
                               &scratch.low_strict_greater_count,
                               scratch.wave_totals, scratch.wave_offsets);
    if (tid == 0) {
        scratch.strict_greater_count += scratch.low_strict_greater_count;
        scratch.equal_needed =
            scratch.remaining_in_high_bin - scratch.low_strict_greater_count;
        scratch.greater_output_count = 0;
        scratch.equal_output_count = 0;
    }
    __syncthreads();
    ...
    const auto threshold = static_cast<uint32_t>(
        (static_cast<uint16_t>(scratch.threshold_high) << 8) |
        static_cast<uint16_t>(scratch.threshold_low));
    for (Slot slot = static_cast<Slot>(tid); slot < slots;
         slot += static_cast<Slot>(kThreads)) {
        uint32_t key;
        int32_t index;
        ...
        if (key > threshold) {
            ...
            if (position < scratch.equal_needed) {
                writer.store(scratch.strict_greater_count + position, key,
                             index);
            }
        }
    }
...
__host__ __device__ __forceinline__ int32_t choose_uniform_float_parts_for_row(
    int32_t length, int32_t rows, int32_t max_parts, bool prefill = false) {
    const auto chosen = nvidia_topk_plan::parts(length, rows, prefill);
    return chosen < max_parts ? chosen : max_parts;
}
...
// Static-stride multi-CTA execution for native 16-bit scores. Local selectors
// publish K exact (key, index) pairs; the last CTA selects the exact row TopK
// from their union. The score tensor remains FP16/BF16 throughout.
template <int MaxParts>
__global__
__launch_bounds__(kThreads) void nvidia_indexer_topk_16_uniform_device_plan(
    TopKParams params, const int32_t *__restrict__ plan_parts, int32_t rows,
    uint64_t *__restrict__ candidates, uint32_t *__restrict__ completion) {
    static_assert(MaxParts == 2 || MaxParts == 4 || MaxParts == 8 ||
                  MaxParts == 16);
    __shared__ TopK16BlockScratch scratch;
    ...
    if (raw_hint_parts <= 1) {
        ...
        const auto row = static_cast<int32_t>(blockIdx.x);
        const auto length =
            params.lengths == nullptr ? params.max_length : params.lengths[row];
        ...
        const auto row_start =
            params.row_starts == nullptr ? 0 : params.row_starts[row];
        Dense16Accessor input{
            params.scores + static_cast<int64_t>(row) * params.input_stride +
                row_start,
            length, 0};
        select_topk16_block_exact(input, IndexWriter{output}, kTopK, scratch);
        return;
    }
    ...
    for (auto task = static_cast<uint32_t>(blockIdx.x); task < logical_tasks;
        ...
        const auto length =
            params.lengths == nullptr ? params.max_length : params.lengths[row];
        const auto parts = choose_uniform_float_parts_for_row(
            length, rows, MaxParts, params.prefill);
        ...
        if (parts == 1 || parts > hint_parts || length < parts * kTopK) {
            if (part == 0) {
                ...
                if (length <= kTopK) {
                    ...
                    const auto row_start = params.row_starts == nullptr
                                               ? 0
                                               : params.row_starts[row];
                    Dense16Accessor input{params.scores +
                                              static_cast<int64_t>(row) *
                                                  params.input_stride +
                                              row_start,
                                          length, 0};
                    select_topk16_block_exact(input, IndexWriter{output}, kTopK,
                                              scratch);
                }
                __syncthreads();
            }
        ...
        if (local_length == kTopK) {
            for (int index = threadIdx.x; index < kTopK; index += blockDim.x) {
                const auto key = sortable_key(row_scores[part_begin + index]);
                part_output[index] = (static_cast<uint64_t>(key) << 32) |
                                     static_cast<uint32_t>(part_begin + index);
            }
        } else {
            Dense16Accessor local_input{row_scores + part_begin, local_length,
                                        part_begin};
            select_topk16_block_exact(local_input,
                                      PackedCandidateWriter{part_output}, kTopK,
                                      scratch);
        }
        ...
        if (threadIdx.x == 0) {
            cuda::atomic_ref<uint32_t, cuda::thread_scope_device> counter(
                completion[row]);
            const auto ticket =
                counter.fetch_add(1u, cuda::memory_order_release);
            if (ticket == static_cast<uint32_t>(parts - 1)) {
                is_last = counter.load(cuda::memory_order_acquire) ==
        ...
        ContiguousPackedCandidateAccessor merge_input{row_candidates,
                                                      parts * kTopK};
        select_topk16_block_exact(merge_input, IndexWriter{output}, kTopK,
                                  scratch);
        __syncthreads();
    }
}
...
//
// A split is used only when every local chunk contains at least K values.
// Each local selector contributes exactly K packed candidates.  The last local
// selector for a row performs the exact dense P*K merge in the same CTA. The
// caller must zero completion before every launch, including graph replays, and
// must not share the workspace across concurrently executing streams.
...
template <int MaxParts>
__global__
__launch_bounds__(kThreads) void nvidia_indexer_topk_float_uniform_device_plan(
...
int32_t max_uniform_float_parts(int64_t rows, int64_t width,
                                bool prefill = false) {
    return nvidia_topk_plan::max_parts(rows, width, prefill);
}
...
void launch_uniform_float_static_stride(const TopKFloatParams &params,
    ...
    // The caller holds the scores' device guard. PyTorch caches properties
    // per device; graph replay does not execute this host-side lookup.
    const auto num_sms =
        at::cuda::getCurrentDeviceProperties()->multiProcessorCount;
    TORCH_CHECK(num_sms > 0, "NVIDIA TopK requires a positive device SM count");
    const auto physical_blocks = nvidia_topk_plan::grid_blocks(
        nvidia_topk_plan::config(rows, params.max_length, params.prefill), rows,
        MaxParts, num_sms);
    nvidia_indexer_topk_float_uniform_device_plan<MaxParts>
...
template <int MaxParts>
void launch_uniform_16_static_stride(const TopKParams &params,
                                     uint64_t *candidates, uint32_t *completion,
                                     const int32_t *plan_parts, int32_t rows,
                                     cudaStream_t stream) {
    const auto logical_tasks = static_cast<uint64_t>(rows) * MaxParts;
    TORCH_CHECK(logical_tasks <= UINT32_MAX,
                "16-bit TopK static task count exceeds uint32");
    const auto num_sms =
        at::cuda::getCurrentDeviceProperties()->multiProcessorCount;
    TORCH_CHECK(num_sms > 0, "NVIDIA TopK requires a positive device SM count");
    const auto physical_blocks = nvidia_topk_plan::grid_blocks(
        nvidia_topk_plan::config(rows, params.max_length, params.prefill), rows,
        MaxParts, num_sms);
    nvidia_indexer_topk_16_uniform_device_plan<MaxParts>
        <<<physical_blocks, kThreads, 0, stream>>>(params, plan_parts, rows,
                                                   candidates, completion);
}
...
void launch_uniform_16_static_stride(const TopKParams &params,
                                     uint64_t *candidates, uint32_t *completion,
                                     const int32_t *plan_parts, int32_t rows,
                                     int32_t max_parts, cudaStream_t stream) {
    switch (max_parts) {
    case 2:
        launch_uniform_16_static_stride<2>(params, candidates, completion,
                                           plan_parts, rows, stream);
        break;
    case 4:
        launch_uniform_16_static_stride<4>(params, candidates, completion,
                                           plan_parts, rows, stream);
        break;
    case 8:
        launch_uniform_16_static_stride<8>(params, candidates, completion,
                                           plan_parts, rows, stream);
        break;
    case 16:
        launch_uniform_16_static_stride<16>(params, candidates, completion,
                                            plan_parts, rows, stream);
        break;
    default:
        TORCH_CHECK(false, "unsupported 16-bit TopK MaxParts: ", max_parts);
    }
}

void launch_topk16_direct(const TopKParams &params, int64_t rows,
                          cudaStream_t stream) {
    if (params.max_length <= kTopK) {
        write_all_indices<<<static_cast<unsigned int>(rows), kWriteThreads, 0,
                            stream>>>(params.output);
    } else if (params.max_length <= 4096) {
        nvidia_indexer_topk_short<4096 / kThreads>
            <<<static_cast<unsigned int>(rows), kThreads, 0, stream>>>(params);
    } else if (params.max_length >= 131072 && params.lengths == nullptr &&
               params.row_starts == nullptr) {
        nvidia_indexer_topk<true, 32>
            <<<static_cast<unsigned int>(rows), kThreads, 0, stream>>>(params);
    } else if (params.max_length >= 49152) {
        nvidia_indexer_topk<true, 16>
            <<<static_cast<unsigned int>(rows), kThreads, 0, stream>>>(params);
    } else if (params.max_length >= 16384) {
        nvidia_indexer_topk<true, 4>
            <<<static_cast<unsigned int>(rows), kThreads, 0, stream>>>(params);
    } else {
        nvidia_indexer_topk<false, 1>
            <<<static_cast<unsigned int>(rows), kThreads, 0, stream>>>(params);
    }
}
...
int32_t nvidia_indexer_topk_plan_parts(const std::vector<int32_t> &old_lengths,
    ...
    const auto captured_max_parts =
        max_uniform_float_parts(rows, static_width, prefill);
...
int64_t nvidia_indexer_topk_workspace_candidate_elements_for_shape(
    int64_t rows, int64_t score_width) {
    ...
    return rows *
           static_cast<int64_t>(
               max_uniform_float_parts(rows, score_width, true)) *
           kTopK;
}
...
void nvidia_indexer_topk(const at::Tensor &scores, at::Tensor &output,
    ...
    TORCH_CHECK(scores.scalar_type() == at::ScalarType::Half ||
                    scores.scalar_type() == at::ScalarType::BFloat16 ||
                    scores.scalar_type() == at::ScalarType::Float,
                "NVIDIA indexer TopK is not implemented for dtype ",
                scores.scalar_type(),
    ...
    if (scores.scalar_type() == at::ScalarType::Float) {
        ...
        if (scores.size(1) <= 4096) {
            nvidia_indexer_topk_float_wide_coarse<
                10, kFloatLocalCandidateCapacity, true>
                <<<static_cast<unsigned int>(scores.size(0)), kThreads, 0,
                   stream>>>(params);
        } else if (scores.size(1) <= 32768) {
            nvidia_indexer_topk_float_wide_coarse<
                12, kFloatLocalCandidateCapacity, true>
                <<<static_cast<unsigned int>(scores.size(0)), kThreads, 0,
                   stream>>>(params);
        } else {
            nvidia_indexer_topk_float_wide_coarse<12,
                                                  kFloatLocalCandidateCapacity>
                <<<static_cast<unsigned int>(scores.size(0)), kThreads, 0,
                   stream>>>(params);
        }
        const auto result = cudaGetLastError();
        TORCH_CHECK(result == cudaSuccess, "nvidia_indexer_topk_float failed: ",
    ...
    launch_topk16_direct(params, scores.size(0), stream);

    const auto result = cudaGetLastError();
    TORCH_CHECK(result == cudaSuccess,
                "nvidia_indexer_topk failed: ", cudaGetErrorString(result));
}
...
void nvidia_indexer_topk_with_workspace(
    ...
    TORCH_CHECK(scores.scalar_type() == at::ScalarType::Half ||
                    scores.scalar_type() == at::ScalarType::BFloat16 ||
                    scores.scalar_type() == at::ScalarType::Float,
                "explicit-workspace NVIDIA indexer TopK is not implemented "
                "for dtype ",
                scores.scalar_type(),
                "; supported dtypes are float16, bfloat16 and float32");
    TORCH_CHECK(scores.size(0) <= INT32_MAX, "TopK batch size exceeds int32");
    TORCH_CHECK(scores.size(1) <= INT32_MAX,
                "score length exceeds the int32 index limit");
    TORCH_CHECK(scores.size(1) >= kTopK, "score length must be at least 2048");
    ...
    const auto max_parts =
        max_uniform_float_parts(scores.size(0), scores.size(1), prefill);
    ...
    if (scores.scalar_type() == at::ScalarType::Float) {
        ...
        if (max_parts == 1) {
            if (scores.size(1) <= kTopK) {
                write_all_indices<<<static_cast<unsigned int>(scores.size(0)),
                                    kWriteThreads, 0, stream>>>(
                    output.data_ptr<int32_t>());
            } else if (nvidia_topk_plan::use_pure_p1(scores.size(0),
                                                     scores.size(1), prefill)) {
                nvidia_indexer_topk_float_pure_p1<<<
                    static_cast<unsigned int>(scores.size(0)), kThreads, 0,
                    stream>>>(params);
            } else if (scores.size(1) <= 4096) {
                nvidia_indexer_topk_float_wide_coarse<
                    10, kFloatLocalCandidateCapacity>
                    <<<static_cast<unsigned int>(scores.size(0)), kThreads, 0,
                       stream>>>(params);
            } else {
                nvidia_indexer_topk_float_wide_coarse<
                    12, kFloatLocalCandidateCapacity>
                    <<<static_cast<unsigned int>(scores.size(0)), kThreads, 0,
                       stream>>>(params);
            }
        } else {
        ...
        if (max_parts == 1) {
            ...
            launch_uniform_16_static_stride(
                params,
                reinterpret_cast<uint64_t *>(candidates.data_ptr<int64_t>()),
                reinterpret_cast<uint32_t *>(completion.data_ptr<int32_t>()),
                plan_parts.data_ptr<int32_t>(),
    ...
    const auto result = cudaGetLastError();
    TORCH_CHECK(result == cudaSuccess,
                "nvidia_indexer_topk_uniform_device_plan failed: ",
                cudaGetErrorString(result));
}
