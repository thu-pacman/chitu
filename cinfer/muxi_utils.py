...
try:
    ...
    class ReportErrorWhenUsed:
        def __getattr__(self, item):
            raise ImportError(
                "Optional dependency 'muxi_layout_kernels' is not installed."
            )

    muxi_layout_kernels = ReportErrorWhenUsed()


def preprocess_weights_for_native_layout(
    checkpoint, model_parallel_size: int, rpl_names: List[str], cpl_names: List[str]
):
    ...
    for key in checkpoint.keys():
        if is_rpl_weight(key):  # Row parallel
            ...
            new_checkpoint[key] = (
                ...
                .permute(2, 0, 3, 1, 4)
                .contiguous()
                .reshape(model_parallel_size, m, k // model_parallel_size)
                .permute(1, 0, 2)
                .contiguous()
                .reshape(m, k)  # Reshape back to for compatibility
            )
...
def linear_layout_native_x_contig_y(x_transposed, w, b=None):
    ...
    w_transposed = w.view(w.shape[0] // 16, w.shape[1] // 8, 16, 8)
    if x_is_vector:
        y = muxi_layout_kernels.gemv_layoutA(w_transposed, x_transposed.view(1, -1))
        # TODO: support bias
        if b is not None:
            y += b
    elif x_transposed.shape[1] * 16 > 256:
