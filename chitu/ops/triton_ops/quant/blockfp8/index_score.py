...
from chitu.ops.triton_ops.utils import SIGNED_INT32_0x87F00000, to_triton_dtype
...
@triton.jit
def softfp8_blockfp8_index_score_ragged_q_dense_k_dsv32_triton_kernel(
    q_ptr,
    q_s_ptr,
    k_ptr,
    k_s_ptr,
    ...
    # Load q as uint8 and soft decode to compute_dtype
    q_offset = pid_bm * stride_qbm
    q_ptrs = (
        q_ptr
        + q_offset
        + tl.arange(0, H)[:, None] * stride_qh
        + tl.arange(0, D)[None, :] * stride_qd
    )
    q_raw = tl.load(q_ptrs)
    q_int = q_raw.to(tl.int8, bitcast=True).to(tl.int32)
    q_int = (q_int << 20) & SIGNED_INT32_0x87F00000
    q = (q_int.to(tl.float32, bitcast=True) * fp8_to_fp32_scale).to(compute_dtype)
    ...
    # Load k as uint8 and soft decode to compute_dtype
    k_ptrs = (
        k_ptr
        + q_seq_id * stride_kb
        + n_idx[:, None] * stride_kn
    ...
    k_raw = tl.load(k_ptrs, mask=n_mask[:, None])
    k_int = k_raw.to(tl.int8, bitcast=True).to(tl.int32)
    k_int = (k_int << 20) & SIGNED_INT32_0x87F00000
    k = (k_int.to(tl.float32, bitcast=True) * fp8_to_fp32_scale).to(compute_dtype)

    # Load k_s
    k_s_ptrs = k_s_ptr + q_seq_id * stride_ksb + n_idx * stride_ksn
    k_s = tl.load(k_s_ptrs, mask=n_mask)

    # Compute GEMM in compute_dtype: K_chunk @ Q^T -> [BLOCK_N, H]
    logits = tl.dot(k, tl.trans(q))
...
def softfp8_blockfp8_index_score_ragged_q_dense_k_dsv32_triton(
    q: torch.Tensor,  # [bm, h=64, d=128], fp8
    q_s: torch.Tensor,  # [bm, h=64, d/block_size=1], fp32
    k: torch.Tensor,  # [b, n, d=128], fp8
    k_s: torch.Tensor,  # [b, n, d/block_size=1], fp32
    ...
    softfp8_blockfp8_index_score_ragged_q_dense_k_dsv32_triton_kernel[grid](
        q.view(dtype=torch.uint8),
        q_s,
        k.view(dtype=torch.uint8),
        k_s,
        o,
        seq_len_delta.delta_seq_ids_tensor_device,
        seq_len_delta.delta_position_ids_tensor_device,
...
@triton.jit
def softfp8_blockfp8_index_score_ragged_q_paged_k_dsv32_triton_kernel(
    q_ptr,
    q_s_ptr,
    k_ptr,
    k_s_ptr,
...
def softfp8_blockfp8_index_score_ragged_q_paged_k_dsv32_triton(
    q: torch.Tensor,  # [bm, h=64, d=128], fp8
    q_s: torch.Tensor,  # [bm, h=64, d/block_size=1], fp32
    k: torch.Tensor,  # [b, n, d=128], fp8
    k_s: torch.Tensor,  # [b, n, d/block_size=1], fp32
    ...
    softfp8_blockfp8_index_score_ragged_q_paged_k_dsv32_triton_kernel[grid](
        q.view(dtype=torch.uint8),
        q_s,
        k.view(dtype=torch.uint8),
        k_s,
