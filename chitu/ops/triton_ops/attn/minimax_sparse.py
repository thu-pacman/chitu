...
#
# Adapted from vLLM (Apache-2.0): vllm/models/minimax_m3/common/ops/sparse_attn.py
...
_TRITON_JIT_PARAMS = inspect.signature(triton.jit).parameters


def _minimax_sparse_triton_jit(**kwargs):
    """Triton jit wrapper; maps alignment flags for older Triton builds."""
    if (
        "do_not_specialize_on_alignment" in kwargs
        and "do_not_specialize_on_alignment" not in _TRITON_JIT_PARAMS
    ):
        align_args = kwargs.pop("do_not_specialize_on_alignment") or []
        existing = list(kwargs.get("do_not_specialize") or [])
        kwargs["do_not_specialize"] = existing + list(align_args)
...
@_minimax_sparse_triton_jit(do_not_specialize_on_alignment=["seq_lens", "prefix_lens"])
