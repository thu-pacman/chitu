...
class PDSchedulerService:
    ...
    async def start(self):
        ...
        if self.scheduler is not None and hasattr(self.scheduler, "set_cache_manager"):
    ...
    async def _init_sockets(self):
        ...
        cfg_base_port = getattr(self.args.dp_config, "scheduler_base_port", None)
    ...
    async def _run_decode_warmup(self, steps: int):
        ...
        try:
            ...
            # Hooks module might not be available or executor not ready
            pass
        ...
        try:
            ...
            for _ in range(steps):
                logits = Backend.executor.decode_step_tp_only([req_id], [last_token])
...
async def start_pd_scheduler_service(args, rank: int = 0):
    ...
    from chitu.serve.event_loop import _server_event_loop
