        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # noise_pred = self.run_controlnet(i, latents, latent_model_input, t, prompt_embeds, add_text_embeds, add_time_ids,
                #        image, controlnet_conditioning_scale, controlnet_keep, guess_mode, added_cond_kwargs,
                #        timestep_cond, guidance_scale)
                import torchperf
                ...
                gm, fx_args = torchperf.torch_dynamo.get_dynamo_graph_modules_and_args(
                    self.run_controlnet, args, kwargs, full_graph=True, dynamic=False
                )
                for i in range(len(gm)):
                    gm, fx_args = gm[i], fx_args[i]
                    gm.graph.print_tabular()
                    torchperf.torch_dynamo.draw_simple_graph(
                        gm, f"controlnet_unet_{i}.svg"
                    )
                exit()
