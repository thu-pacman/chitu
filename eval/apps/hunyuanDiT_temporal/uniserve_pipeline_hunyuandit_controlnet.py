...
class HunyuanDiTControlNetPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        controlnet_start_end_point: Optional[Union[int, int]] = None,
        hash_key: int = 0,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i < controlnet_start_end_point[0]:
                ...
                # if False:
                if (
                    i < controlnet_start_end_point[0]
                    or i > controlnet_start_end_point[1]
                ):
                    control_block_samples = [
                        torch.zeros(batch_size * 2, 4096, 1408).to(
                ...
                # INSERT_YOUR_CODE
                # print(f"Type of control_block_samples: {type(control_block_samples)}")
                # for idx, block in enumerate(control_block_samples):
                #     if hasattr(block, 'shape'):
                #         print(f"Shape of control_block_samples[{idx}]: {block.shape} {block.dtype}")
                #     else:
                #         print(f"control_block_samples[{idx}] has no 'shape' attribute.")
