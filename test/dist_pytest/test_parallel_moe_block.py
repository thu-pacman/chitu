...
from chitu.device_type import has_native_fp8, is_ascend_910b, is_hygon
...
_, has_aiter_moe = try_import_platform_dep("aiter.moe")
lightop, has_lightop = try_import_platform_dep("lightop")
_, has_hygon_deepgemm = try_import_opt_dep("deepgemm", "deepgemm_hygon")
...
def _quantize_w8a8_weight(weight: torch.Tensor):
    scales = (weight.abs().amax(dim=-1) / 127.0).clamp(min=1e-6).to(weight.dtype)
    qweight = torch.round(weight / scales.unsqueeze(-1)).clamp(-128, 127)
    return qweight.to(torch.int8).contiguous(), scales.contiguous()


@functools.cache
def gen_parallel_moe_block_input_w8a8_per_token_per_channel_dyn(
    *,
    batch_size,
    hidden_dim,
    n_routed_experts,
    ...
    experts_shape = (n_routed_experts + n_fused_shared_experts,)
    gate_weight = torch.randn(
        *experts_shape, moe_inter_dim, hidden_dim, dtype=dtype, device="cuda"
    )
    up_weight = torch.randn(
        *experts_shape, moe_inter_dim, hidden_dim, dtype=dtype, device="cuda"
    )
    down_weight = torch.randn(
        *experts_shape, hidden_dim, moe_inter_dim, dtype=dtype, device="cuda"
    )
    global_experts_gate_qweight, global_experts_gate_scale = _quantize_w8a8_weight(
        gate_weight
    )
    global_experts_up_qweight, global_experts_up_scale = _quantize_w8a8_weight(
        up_weight
    )
    global_experts_down_qweight, global_experts_down_scale = _quantize_w8a8_weight(
        down_weight
    )
...
def _w8a8_per_token_per_channel_dyn_moe_experts_cls(experts_impl: str):
    if experts_impl == "torch_npu":
        return AscendW8A8PerTokenPerChannelDynMoeExperts
    if experts_impl == "triton":
        return TritonW8A8PerTokenPerChannelDynMoeExpertsMerged
    if experts_impl == "hygon_aiter":
        return HygonAiterW8A8PerTokenPerChannelDynMoeExpertsMerged
    if experts_impl == "hygon_lightop":
        return HygonLightopW8A8PerTokenPerChannelDynMoeExpertsMerged
    if experts_impl == "hygon_deepgemm":
        return HygonDeepGemmW8A8PerTokenPerChannelDynMoeExpertsMerged
    raise NotImplementedError


@functools.cache
def ref_parallel_moe_block_w8a8_per_token_per_channel_dyn(
    *,
    singleton_group,
    experts_impl,
    n_routed_experts,
    ...
    ref_moe_impl.prepare(task_type, batch_size)
    moe_experts_cls = _w8a8_per_token_per_channel_dyn_moe_experts_cls(experts_impl)
    with torch.device("meta"):
        ref_moe_block = ParallelMoeBlock(
            ...
            moe_experts_cls(
                dim=hidden_dim,
                moe_inter_dim=moe_inter_dim,
                global_n_experts=n_routed_experts + n_fused_shared_experts,
                experts_start_idx=0,
                experts_end_idx=n_routed_experts + n_fused_shared_experts,
                n_activated_experts=topk,
                checkpoint_prefix="ffn.experts",
            ),
            non_fused_shared_experts=None,
    ...
    ref_state_dict = {
        "gate.weight": global_gate_weight,
        "experts.gate_up_proj_weight": torch.cat(
            [global_experts_gate_qweight, global_experts_up_qweight], dim=1
        ),
        "experts.gate_up_proj_weight_scale": torch.cat(
            [global_experts_gate_scale, global_experts_up_scale], dim=1
        ),
        "experts.down_proj_weight": global_experts_down_qweight,
        "experts.down_proj_weight_scale": global_experts_down_scale,
    }
    ref_moe_block.load_state_dict(ref_state_dict, strict=True, assign=True)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [2, 1, 1, 2],
        [1, 2, 1, 2],
        # NOTE: We skip etp_size>1 cases here, because ETP does not gurantee
        # equivalent tensor result when used together with a per-token quantization
        # method, because the quantization group would span over the entire hidden
        # dimension without ETP, but only span over the local hidden dimension with
        # ETP. This is acceptable for real models, because smaller quantization
        # group usually have even better accuracy, but we can't test it by comparing
        # tensor results here.
    ],
)
@pytest.mark.parametrize("batch_size", [0, 1, 16])
...
@pytest.mark.parametrize("dtype", [torch.bfloat16])
@pytest.mark.parametrize(
    "token_dispatcher_impl", [None, "allgather", "npu_all_to_all", "npu_distribute"]
)
@pytest.mark.parametrize(
    "experts_impl",
    ["torch_npu", "triton", "hygon_aiter", "hygon_lightop", "hygon_deepgemm"],
)
def test_parallel_moe_block_w8a8_per_token_per_channel_dyn(
    tp_size,
    dp_size,
    etp_size,
    ep_size,
    ...
    if task_type == TaskType.Decode and token_dispatcher_impl == "allgather":
        pytest.skip(f"{token_dispatcher_impl} is only used for prefill")
    if task_type == TaskType.Prefill and token_dispatcher_impl == "npu_distribute":
        pytest.skip(f"{token_dispatcher_impl} is only for decode")
    if is_ascend_910b() and tp_size > 1 and token_dispatcher_impl == "npu_distribute":
        pytest.skip("npu_distribute with TP>1 on Ascend 910B is not supported")
    if (
    ...
    if experts_impl == "triton" and not has_triton:
        pytest.skip("triton is not available")
    if experts_impl.startswith("hygon_") and not is_hygon():
        pytest.skip("Hygon W8A8 backends require Hygon platform")
    if experts_impl == "hygon_aiter":
        if not has_aiter_moe:
            pytest.skip("aiter.moe is not available")
        if n_fused_shared_experts > 0:
            pytest.skip(
                "hygon_aiter does not support the shape with fused shared experts"
            )
        if ep_size > 1:
            pytest.skip(
                "Currently hygon_aiter adapter in chitu requires all experts to be local"
            )
    if experts_impl == "hygon_lightop" and not (
        has_lightop
        and hasattr(lightop, "gemm_w8a8_smooth")
        and hasattr(lightop, "moe_gemm_w8a8")
    ):
        pytest.skip("lightop W8A8 MoE is not available")
    if experts_impl == "hygon_deepgemm" and not has_hygon_deepgemm:
        pytest.skip("hygon deepgemm W8A8 MoE is not available")
    ...
    (
        ...
        global_experts_down_qweight,
        global_experts_down_scale,
    ) = gen_parallel_moe_block_input_w8a8_per_token_per_channel_dyn(
        batch_size=batch_size,
        hidden_dim=hidden_dim,
        n_routed_experts=n_routed_experts,
        moe_inter_dim=moe_inter_dim,
    ...
    ref_y = ref_parallel_moe_block_w8a8_per_token_per_channel_dyn(
        singleton_group=singleton_group,
    ...
    if rank < test_world_size:
        ...
        n_slots = (
            int(math.ceil((n_routed_experts + n_fused_shared_experts) / ep_size))
            * ep_size
        )
        experts_start_idx = ep_group.rank_in_group * n_slots // ep_size
        experts_end_idx = (ep_group.rank_in_group + 1) * n_slots // ep_size
        ...
        state_dict = {
            ...
            "experts.down_proj_weight_scale": experts_down_scale_in_local_slots.contiguous(),
        }
        parallel_moe_block.load_state_dict(state_dict, strict=True, assign=True)
