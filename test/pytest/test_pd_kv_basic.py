...
def test_pd_transfer_end_to_end(
    ...
    # Prefill side
    prefill_cache = _build_cache_manager(device=device)
    prefill_cache.prepare_cache_prefill([req_id], [prefix_len])
    prefill_meta = MetadataBuffers(size=4)
    prefill_kv = KVManager(
        cache_manager=prefill_cache,
        metadata_buffers=prefill_meta,
        disaggregation_mode=DisaggregationMode.PREFILL,
    )

    # Decode side
    decode_cache = _build_cache_manager(device=device)
    decode_meta = MetadataBuffers(size=4)
    decode_kv = KVManager(
        cache_manager=decode_cache,
    ...
    decode_kv.prepare_kv_transfer([req_id], decode_cache, [prefix_len])
    ...
    _ = decode_kv.recv_kv_cache_and_insert(
        request_ids=[req_id], cache_manager=decode_cache, prefix_lens=[prefix_len]
    )

    assert req_id in decode_cache.req_id_to_seq_len
