...
@pytest.mark.skipif(
    ...
    reason="This test requires NVIDIA GPU with compute capability >= 9.0",
)
def test_quant_einsum_shd_hdc_shc():
    q_nope = torch.randn((22, 16, 128), dtype=torch.bfloat16, device="cuda")
    weight = torch.randn((16, 128, 512), dtype=torch.bfloat16, device="cuda").to(
        torch.float8_e4m3fn
    )
    scale = torch.randn((16, 1, 4), dtype=torch.float32, device="cuda")
    torch_out = quant_einsum_shd_hdc_shc(
        q_nope.to(torch.float), weight, scale, impl="torch"
    )
    triton_out = quant_einsum_shd_hdc_shc(q_nope, weight, scale, impl="triton")
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        x_vals=list(
            itertools.product(bench1_M, bench1_K, bench1_N)
        ...
        plot_name="shd,hdc->shc performance",  # name for the plot. Used also as a file name for saving the plot.
        args={},
    )
)
def benchmark_quant_einsum_shd_hdc_shc(M, K, N, provider):
    q_nope = torch.randn((M, 16, K), dtype=torch.bfloat16, device="cuda")
    weight = torch.randn((16, K, N), dtype=torch.bfloat16, device="cuda").to(
        torch.float8_e4m3fn
    )
    scale = torch.randn((16, K // 128, N // 128), dtype=torch.float32, device="cuda")
    DEVICE = q_nope.device
    stream = getattr(torch, DEVICE.type).Stream()
    getattr(torch, DEVICE.type).set_stream(stream)
    if provider == "torch":
        ms = triton.testing.do_bench(
            lambda: quant_einsum_shd_hdc_shc(
                q_nope.to(torch.float), weight, scale, impl="torch"
            )
        )
    if provider == "triton":
        ms = triton.testing.do_bench(
            lambda: quant_einsum_shd_hdc_shc(q_nope, weight, scale, impl="triton")
        )
    return ms
...
def test_quant_einsum_shc_hdc_shd():
    q_nope = torch.randn((22, 16, 128), dtype=torch.bfloat16, device="cuda")
    weight = torch.randn((16, 512, 128), dtype=torch.bfloat16, device="cuda").to(
        torch.float8_e4m3fn
    )
    ...
    triton_out = quant_einsum_shc_hdc_shd(q_nope, weight, scale, impl="triton")
    assert check_close(torch_out, triton_out)
...
if __name__ == "__main__":
    test_quant_einsum_shd_hdc_shc()
    benchmark_quant_einsum_shd_hdc_shc.run(show_plots=False, print_data=True)
