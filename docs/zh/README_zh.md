...
QwQ-32B 官方发布的原版模型采用 BF16 格式的参数。
我们使用社区上提供的 Q8_0 和 AWQ 量化版本，以及我们自己制作的 [FP8 量化版本](https://huggingface.co/qingcheng-ai/QWQ-32B-FP8) 与原版进行精度对比测试。
下表中的测试结果基于 MMLU 测试集完成。
为了加快测试速度，我们通过某些方式阻止模型产生过长的思维链，因此测试分数可能与开放思维链场景下测得的分数不同。
在我们的测试结果中，**FP8 量化版本的精度与 BF16 原本几乎没有区别，可以使用 FP8 量化版以加速模型的推理。**
