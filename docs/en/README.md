...
[2025/04/30] Released v0.3.0, added support for online conversion of FP4 to FP8 and BF16, supported the [FP4 quantized version](https://huggingface.co/nvidia/DeepSeek-R1-FP4) of DeepSeek-R1 671B.
...
### Heterogeneous deployment of DeepSeek-R1-671B on Xeon 8480P + H20 (96G) servers (v0.2.2 version data)
...
|Hardware environment|6 Node|3 nodes|
|:---|:---|:---|
|Framework and precision|chitu 0.1.0, BF16|Chitu 0.1.0, FP8|
|Using cuda graph|29.8 output token/s|22.7 output token/s|
|Not using cuda graph|8.5 output token/s|7.0 output token/s|

- The data in the table are the output speed of a single request scenario (bs=1)
- For Chitu, the output speed of the FP8 model using 3 nodes is comparable to the speed of the BF16 model using 6 nodes
- For the output speed of the answer to a single request, whether to use cuda graph has a significant impact on the performance, and the performance is significantly improved after using cuda graph
